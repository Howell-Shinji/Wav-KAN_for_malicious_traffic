...
class KAN(nn.Module):
    def __init__(self, layers_hidden, wavelet_type='mexican_hat', dropout_rate=0, 
                 use_residual=True, activation='gelu', norm_type='Batch'):
        super(KAN, self).__init__()
        self.layers = nn.ModuleList()
        ...
        for i, (in_features, out_features) in enumerate(zip(layers_hidden[:-1], layers_hidden[1:])):
            ...
            # Add residual connections where input/output dimensions match
            if use_residual and in_features == out_features and i > 0:
                self.residual_layers.append(i)
                
        # Final normalization layer
        if norm_type == 'batch':
            self.final_norm = nn.BatchNorm1d(layers_hidden[-1])
        elif norm_type == 'layer':
            self.final_norm = nn.LayerNorm(layers_hidden[-1])
        else:
            self.final_norm = None

    def forward(self, x):
        residuals = {}  # Store residual connections
        
        for i, layer in enumerate(self.layers):
            # Store input for potential residual connection
            if i > 0 and i in self.residual_layers:
                residuals[i] = x
